...
@synthesize useControlView;
@synthesize useScoreView;
...
+ (instancetype)numberTileGameWithDimension:(NSUInteger)dimension
    ...
    c.view.backgroundColor = backgroundColor ?: [UIColor brownColor];
    ...
    c.navigationItem.leftBarButtonItem = backButton;
...
- (void)setupGame {
    ...
    CGFloat cellWidth = floorf((288 - padding*(self.dimension+1))/((float)self.dimension));
    if (cellWidth < 30) {
        cellWidth = 30;
    }
...
- (void)viewDidLoad {
    ...
    swipeUpGesture = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(upButtonTapped)];
    [swipeUpGesture setDirection:UISwipeGestureRecognizerDirectionUp];
    [self.view addGestureRecognizer:swipeUpGesture];
    
    swipeDownGesture = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(downButtonTapped)];
    [swipeDownGesture setDirection:UISwipeGestureRecognizerDirectionDown];
    [self.view addGestureRecognizer:swipeDownGesture];
    
    swipeLeftGesture = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(leftButtonTapped)];
    [swipeLeftGesture setDirection:UISwipeGestureRecognizerDirectionLeft];
    [self.view addGestureRecognizer:swipeLeftGesture];
    
    swipeRightGesture = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(rightButtonTapped)];
    [swipeRightGesture setDirection:UISwipeGestureRecognizerDirectionRight];
    [self.view addGestureRecognizer:swipeRightGesture];
    
    self.navigationController.interactivePopGestureRecognizer.enabled = NO;
    
    dimension = 4; //dimension > 2 ? dimension : 2;
    threshold = 2048; //threshold > 8 ? threshold : 8;
    useScoreView = true; //scoreModuleEnabled;
    useControlView = false; //buttonControlsEnabled;
    //self.view.backgroundColor = [UIColor whiteColor]; //backgroundColor ?: [UIColor whiteColor];
